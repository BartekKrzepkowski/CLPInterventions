...
class TrainerClassification:
    def __init__(self, model, criterion, loaders, optim, lr_scheduler, device, extra):
        self.model = model
        ...
        self.run_stats = RunStatsBiModal(model, optim)
    ...
    def run_exp1(self, config):
        ...
        step = f'{self.global_step}_window_{config.extra["window"]}'
        ...
        global_step_train = (self.epoch + 1) * len(self.loaders['train'])
        step = f'{global_step_train}_window_{config.extra["window"]}'
        save_model(self.model, self.save_path(step))
    ...
    def run_exp2(self, config):
        ...
        if config.extra['window'] != 0:
            ...
            self.criterion.fpw = 0.0
            self.loaders['train'].dataset.transform = TRANSFORMS_NAME_MAP['transform_train_blurred']
    ...
    def at_exp_start(self, config):
        ...
        # self.logger.log_graph(self.model, self.criterion)
            ...
            if 'train' == phase:
                ...
                if (i + 1) % config.grad_accum_steps == 0 or (i + 1) == loader_size:
                    ...
                    if self.run_stats is not None:
                        step_assets['evaluators'] = self.run_stats(step_assets['evaluators'], 'l2', self.global_step)
